  ...
  try {
    if(charactersController.position.atEdge){
      if(charactersController.position.pixels != 0){
        pageNo++;
        BlocProvider.of<CharactersBloc>(context).add(GetCharactersEvent(isRefreshed:true, page:pageNo, stopSearching: false));
      }
    }
...
int pageNo = 1;
  ...
  void initState() {
    super.initState();
    BlocProvider.of<CharactersBloc>(context).add(GetCharactersEvent(isRefreshed:false, page:pageNo, stopSearching: false));
